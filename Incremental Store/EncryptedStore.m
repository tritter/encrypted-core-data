    ...
    [[mapping relationshipMappings] enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        if ([relationship isToMany] && [relationship.inverseRelationship isToMany] && [destinationRelationship isToMany] && [destinationRelationship.inverseRelationship isToMany])
        {
            ...
            if (tableExists)
            {
                ...
                NSString *firstIDColumn;
                NSString *secondIDColumn;
                [self relationships:destinationRelationship firstIDColumn:&firstIDColumn secondIDColumn:&secondIDColumn];
                
                NSString *previousFirstIDColumn;
                NSString *previousSecondIDColumn;
                [self previousRelationships:destinationRelationship firstIDColumn:&previousFirstIDColumn secondIDColumn:&previousSecondIDColumn];
                ...
                string = [NSString stringWithFormat:
                          ...
                          @"FROM %@",
                          newTableName,
                          [@[firstIDColumn, secondIDColumn] componentsJoinedByString:@", "],
                          [@[previousFirstIDColumn, previousSecondIDColumn] componentsJoinedByString:@", "],
                          temporaryTableName];
                statement = [self preparedStatementForQuery:string];
                sqlite3_step(statement);
                if (statement == NULL || sqlite3_finalize(statement) != SQLITE_OK) {
...
-(BOOL)previousRelationships:(NSRelationshipDescription *)relationship firstIDColumn:(NSString *__autoreleasing*)firstIDColumn secondIDColumn:(NSString *__autoreleasing*)secondIDColumn
{
